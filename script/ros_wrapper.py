...
class WTAOptimization():
    def __init__(self):
        ...
        self.k_max = rospy.get_param("max_iter", 100)
        self.publishing_threshold = rospy.get_param("pub_prob", 0.2)
        # my_namespace = rospy.get_namespace()
        my_namespace = "robot01" # adding for  debugging
    ...
    def optimization(self):
        ...
        convdiff = [1]
        k = 0
        while convdiff[k] > 10 ** -8 and not self.stop_optimization and not rospy.is_shutdown() and k < self.k_max:
        ...
        # if convdiff[k] < 10 ** -8: # if we have reached convergence, I think we should continue optimzing over and over to make sure we are assigned to the same target.
